...
let _timer = null   //long call for connect
...
class Start1 extends Component {
    ...
    onMcUp = () => {
        const {connected, currentMcVersion, spManager} = this.state
    ...
    componentDidMount() {
        let {connected, currentMcVersion, spManager} = this.props

        if (!connected) {
            let spManager = new SPManager(
                () => {
            ...
            //scan if serial is ready ,auto jump to the upgrade page
			let scanTime=0
            const ifConnect = () => {
                const {connected, currentMcVersion, spManager} = this.state
				const {_scanTimer}=this.props
                if (connected) {
					...
					if(scanTime>4){
						...
						this.props.hasSetScanTime(true)
						clearInterval(_timer)
						_timer = null
					}
                }
            }
            _timer = setInterval(ifConnect, 500)
    ...
    render() {
        ...
        return (
            <div className={s1['warper']}>
                ...
                {showTips ? <div className={s1['hmAlert']}>
                    ...
                    <ul className={s1['hUl']}>
                        ...
                        <li>{messages['l1_for_31']/* 方式三： */}<span className={s1['hLink']} onClick={this.installDriver}>{messages['l1_for_32']/* 点击安装驱动 */}</span></li>
                        <li>{messages['l1_for_4']}{/* 方式四：关掉其他软件，或者拔掉电脑其他串口设备 */}</li>
                        <li>{messages['l1_for_51']}{/* 方法五：检查主控是否一直闪红灯，如果一直闪红灯， */}<span className={s1['hLink']} onClick={this.onMcUp}>{messages['l1_for_52']}{/* 点击升级主控 */}</span>
                        </li>
                    </ul>
                </div> : null}
                {
                    checkLink?<div className={s1['hcAlert']}>
                        <div  className={s1['hcHeader']}>
                            {/*提示*/} {messages['tips']}
                            <span onClick={this.hideAlert} className={s1['hcClose']}>×</span>
                        </div>
                        <div className={s1['hcTip']}>{/*请检查USB线是否连接正确*/}  {messages['l1_if_connect']}</div>
                        <div className={s1['ucButton']} onClick={ this.closeCheckLink}>{/*确定*/}{messages['yes']}</div>
                    </div> : null
                }
				 {
				    showNoPlatfrom?<div className={s1['hcAlert']}>
				        <div  className={s1['hcHeader']}>
                            {/*提示*/} {messages['tips']}
				            <span onClick={()=>{this.setState({showNoPlatfrom:false})}} className={s1['hcClose']}>×</span>
				        </div>
				        <div className={s1['hcTip']}>{messages['noPlatform']}</div>
				        <div className={s1['ucButton']} onClick={ ()=>{this.setState({showNoPlatfrom:false})}}>{/*确定*/}{messages['yes']}</div>
				    </div> : null
				}
				
				 {installDrive?<div className={c(s1['hrAlert'],s1['install'])} >
				    <div className={s1['hcHeader']}>
				        {/*提示*/} {messages['tips']}
				        <span onClick={() => {
				            this.setState({
                ...
                {showFreRed?<div className={s1['hrAlert']} >
                    <div className={s1['hcHeader']}>
                        {/*提示*/} {messages['tips']}
                        <span onClick={() => {
                            this.setState({
